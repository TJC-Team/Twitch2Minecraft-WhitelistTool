...
function parseIRCMessage(messageSrc) {
    ...
    if (messageSrc.startsWith("@")) {
        ...
        if (spaceIdx < 0) {
            // not found
            throw new parse_error_1.ParseError(`No space found after tags declaration (given src: "${messageSrc}")`);
        }
        const tagsSrc = remainder.slice(0, spaceIdx);
        if (tagsSrc.length === 0) {
            throw new parse_error_1.ParseError(`Empty tags declaration (nothing after @ sign) (given src: "${messageSrc}")`);
        }
    ...
    if (remainder.startsWith(":")) {
        ...
        if (spaceIdx < 0) {
            // not found
            throw new parse_error_1.ParseError(`No space found after prefix declaration (given src: "${messageSrc}")`);
        }
        ircPrefixRaw = remainder.slice(0, spaceIdx);
        remainder = remainder.slice(spaceIdx + 1);
        if (ircPrefixRaw.length === 0) {
            throw new parse_error_1.ParseError(`Empty prefix declaration (nothing after : sign) (given src: "${messageSrc}")`);
        }
        if (!ircPrefixRaw.includes("@")) {
            ...
            if (host.length === 0 ||
                ...
                throw new parse_error_1.ParseError(`Host, nick or user is empty in prefix (given src: "${messageSrc}")`);
            }
            ircPrefix = {
                nickname: nick,
                username: user,
    ...
    if (spaceAfterCommandIdx < 0) {
        ...
        while (paramsRemainder !== null) {
            if (paramsRemainder.startsWith(":")) {
                ...
                if (param.length === 0) {
                    throw new parse_error_1.ParseError(`Too many spaces found while trying to parse middle parameters (given src: "${messageSrc}")`);
                }
    ...
    if (!VALID_CMD_REGEX.test(ircCommand)) {
        throw new parse_error_1.ParseError(`Invalid format for IRC command (given src: "${messageSrc}")`);
    }
